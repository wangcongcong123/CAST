...
This script is adapted from: https://github.com/huggingface/transformers/blob/master/src/transformers/trainer.py
'''
...
import os
# added when switching to transformers.v.4.0.0
...
# comment when switching to transformers.v.4.0.0
...
from ptt.utils import save_ck, get_optimizer, get_scheduler, count_params
from ptt.utils import write_args, print_few_examples
from ptt.hf_training_args import HFTrainingArguments
...
class HFTrainer:
    ...
    def __init__(
        ...
        if logger is None:
            self.logger = hf_logging.get_logger(__name__)
        else:
            self.logger = logger

        if args is None:
            self.logger.info("No `TrainingArguments` passed, using the current path as `output_dir`.")
            args = HFTrainingArguments("tmp_trainer")
        self.args = args
        self.logger.info(f" see seed for random, numpy and torch {args.seed}")
        ...
        if tb_writer is None and is_tensorboard_available() and self.is_world_process_zero():
            if self.args.logging_dir is None:
                ...
                self.tb_writer = SummaryWriter(
                    log_dir=self.args.logging_dir) if self.args.logging_dir.strip() != "" else None

        if not is_tensorboard_available() and self.tb_writer is not None:
            logger.warning(
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.optimizer is None:
            ...
            self.optimizer = get_optimizer(self.args, optimizer_grouped_parameters)
        ...
        if self.lr_scheduler is None:
            self.lr_scheduler = get_scheduler(self.optimizer, self.args.scheduler, self.args.warmup_steps,
                                              num_training_steps)
    ...
    def setup_wandb(self):
        ...
        if self.is_world_process_zero():
            ...
            wandb.init(
                project=self.args.wandb_project_name, config=combined_dict, name=self.args.wandb_run_name
            )
    ...
    def setup_comet(self):
        ...
        if self.is_world_process_zero():
            ...
            args = {"project_name": self.args.comet_project_name, "api_key": self.args.comet_api_key}
            comet_mode = self.args.comet_mode.upper()
            experiment = None
            if comet_mode == "ONLINE":
                experiment = comet_ml.Experiment(**args)
                self.logger.info("Automatic Comet.ml online logging enabled")
            elif comet_mode == "OFFLINE":
                args["offline_directory"] = os.getenv("COMET_OFFLINE_DIRECTORY", "./")
                experiment = comet_ml.OfflineExperiment(**args)
                self.logger.info("Automatic Comet.ml offline logging enabled; use `comet upload` when finished")
    ...
    def _setup_loggers(self):
        if self._loggers_initialized:
            return
        if self.args.use_wandb and is_wandb_available():
            self.setup_wandb()
        ...
        if self.args.use_comet and is_comet_available():
            self.setup_comet()
    ...
    def _hp_search_setup(self, trial: Union["optuna.Trial", Dict[str, Any]]):
        ...
        if self.hp_search_backend == HPSearchBackend.OPTUNA:
            self.logger.info("Trial:", trial.params)
    ...
    def _tune_save_checkpoint(self):
        ...
        with tune.checkpoint_dir(step=self.global_step) as checkpoint_dir:
            self.args.output_path = checkpoint_dir
            output_dir = os.path.join(self.args.output_path, f"{PREFIX_CHECKPOINT_DIR}-{self.global_step}")
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None,
        ...
        if self.args.warmup_steps_or_ratio <= 1 and self.args.warmup_steps_or_ratio > 0:
            self.args.warmup_steps = int(t_total * self.args.warmup_steps_or_ratio)
        else:
            self.args.warmup_steps = self.args.warmup_steps_or_ratio
        if self.args.warmup_steps_or_ratio <= 0:
        ...
        model = self.model
        self.logger.info(f"  Number of trainable model parameters: {model.num_parameters(only_trainable=True)}")
        ...
        if self.args.n_gpu > 1:
            if self.args.visible_gpu_devices is not None:
                model = torch.nn.DataParallel(model,
                                              device_ids=[int(i) for i in self.args.visible_gpu_devices.split(",")])
            else:
                model = torch.nn.DataParallel(model)
        ...
        if verbose:
            count_params(model, self.logger, print_details=verbose)
            # bu default, print ten examples for debugging purpose if verbose
            print_few_examples(self.logger, self.train_dataset, tokenizer=self.tokenizer, n=10)

        self.logger.info("***** Running training *****")
        self.logger.info("  Num examples = %d", self.num_examples(train_dataloader))
        # not appropriate approximation for Albert
        # self.logger.info(f"  Number of floating-point operations (forward and backward including embeddings): {model.floating_point_ops()}")
        self.logger.info("  Num Epochs = %d", num_train_epochs)
        self.logger.info("  Instantaneous batch size per device = %d", self.args.per_device_train_batch_size)
        self.logger.info("  Device count (n_gpu) = %d", self.args.n_gpu)
        ...
        self.logger.info("  LR warmup steps = %d", int(self.args.warmup_steps))
        ...
        if self.hp_search_backend != HPSearchBackend.RAY:
            # will raise an error when it is ray, no idea why
            self.logger.info("  Experimental Configuration: ")
            write_args(self.args, logger=self.logger)
        ...
        if model_path is not None:
            ...
            try:
                ...
                self.logger.info("  Continuing training from %d non-embedding floating-point operations",
                                 self.total_flos)
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            epoch_pbar = tqdm(epoch_iterator, disable=disable_tqdm)
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if steps_trained_in_current_epoch > 0:
                    ...
                    epoch_pbar.set_description(
                        f"Training - epoch {epoch + 1}/{self.args.num_train_epochs} iter {step}. lr {self.lr_scheduler.get_last_lr()[0]:e}")
                    continue
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if (self.args.log_and_save_steps > 0 and self.global_step % self.args.log_and_save_steps == 0) or (
                            self.global_step == 1 and self.args.logging_first_step
                    ):
                        self.logger.info(f"report train scores at global_step = {self.global_step}")
                        logs: Dict[str, float] = {}

                        # tr_loss_scalar = tr_loss.item()
                        # logs["train_global_step_loss"] = (tr_loss_scalar - logging_loss_scalar) / self.args.log_and_save_steps

                        mean_train_loss = np.mean(losses)
                        logs["train_global_step_loss"] = mean_train_loss
                        losses = []

                        # backward compatibility for pytorch schedulers
                        logs["train_global_step_learning_rate"] = (
                            self.lr_scheduler.get_last_lr()[0]
                            if version.parse(torch.__version__) >= version.parse("1.4")
                            else self.lr_scheduler.get_lr()[0]
                        )
                        # logging_loss_scalar = tr_loss_scalar
                        self.log(logs, tag="train_global_step")

                    if (
                            self.args.evaluation_strategy == EvaluationStrategy.STEPS
                            and self.global_step % self.args.eval_steps == 0  and self.eval_dataset is not None
                    ):
                        if self.evaluate_fn is not None:
                            eval_dict = self.evaluate_fn(self.args, self.logger, model, self.tokenizer,
                                                         self.get_eval_dataloader(),
                                                         steps=self.global_step, tag="global_step")

                            self.log({"eval_" + key+"_global_step": value for key, value in eval_dict["eval_scores"].items()}, tag="eval_global_step")

                            # not need this anymore, because self.log will take care of this
                            # if self.tb_writer:
                            #     if "eval_scores" in eval_dict:
                            #         for key, value in eval_dict["eval_scores"].items():
                            #             self.tb_writer.add_scalar(f"eval_{key}_global_step", value, self.global_step)

                            if "is_early_stop" in eval_dict and eval_dict["is_early_stop"]:
                                self.logger.info(f"run out of patience at global step = {self.global_step}, early stop")
                                if self.tb_writer:
                                    self.tb_writer.close()
                                early_exit = True
                                break
                            self._report_to_hp_search(trial, self.global_step, eval_dict["eval_scores"])
                        else:
                            metrics = self.evaluate()
                            self._report_to_hp_search(trial, self.global_step, metrics)
                            ...
                            output_dir = os.path.join(self.args.output_path, checkpoint_folder)
                            ...
                            if is_torch_tpu_available():
                                ...
                                torch.save(self.optimizer.state_dict(), os.path.join(output_dir, "optimizer.pt"))
                                torch.save(self.lr_scheduler.state_dict(), os.path.join(output_dir, "scheduler.pt"))
                    elif self.args.evaluation_strategy == EvaluationStrategy.NO and self.global_step % self.args.eval_steps == 0:
                        save_ck(self.args, self.logger, model, tokenizer=self.tokenizer, steps=self.global_step,
                                tag="global_step")


                epoch_pbar.update(1)
                epoch_pbar.set_description(
                    f"Training - epoch {epoch + 1}/{self.args.num_train_epochs} iter {step}: train loss {step_loss.item():.8f}. lr {self.lr_scheduler.get_last_lr()[0]:e}")

                if self.args.max_steps > 0 and self.global_step >= self.args.max_steps:
                    break

            if self.args.evaluation_strategy == EvaluationStrategy.EPOCH:
                # self.logger.info(f'Epoch {epoch + 1} - mean train loss: {np.mean(losses):.3}')
                # self.logger.info(f"evaluate at epoch = {epoch + 1}")
                # if self.tb_writer:
                #     self.tb_writer.add_scalar("train_loss_epoch", np.mean(losses), epoch + 1)
                #     self.tb_writer.add_scalar("train_lr_epoch", self.lr_scheduler.get_last_lr()[0], epoch + 1)
                # losses = []
                self.logger.info(f"report train scores at epoch = {epoch + 1}")
                logs: Dict[str, float] = {}
                mean_train_loss = epoch_loss/(step+1)
                logs["train_epoch_loss"] = mean_train_loss
                # backward compatibility for pytorch schedulers
                logs["train_epoch_learning_rate"] = (
                    self.lr_scheduler.get_last_lr()[0]
                    if version.parse(torch.__version__) >= version.parse("1.4")
                    else self.lr_scheduler.get_lr()[0]
                )
                logging_loss_scalar = mean_train_loss
                self.log(logs, tag="train_epoch")
                losses = []
                if self.evaluate_fn is not None and self.eval_dataset is not None:
                    eval_dict = self.evaluate_fn(self.args, self.logger, model, self.tokenizer,
                                                 self.get_eval_dataloader(),
                                                 steps=epoch + 1, tag="epoch")
                    self.log({"eval_" + key+"_epoch": value for key, value in eval_dict["eval_scores"].items()}, tag="eval_epoch")
                    # not need this anymore, because self.log will take care of this
                    # if self.tb_writer:
                    #     if "eval_scores" in eval_dict:
                    #         for key, value in eval_dict["eval_scores"].items():
                    #             self.tb_writer.add_scalar(f"eval_{key}_epoch", value, epoch + 1)
                    if "is_early_stop" in eval_dict and eval_dict["is_early_stop"]:
                        self.logger.info(f"run out of patience at epoch = {epoch + 1}, early stop")
                        if self.tb_writer:
                            self.tb_writer.close()
                        break
                    self._report_to_hp_search(trial, epoch + 1, eval_dict["eval_scores"])
                elif self.compute_metrics is not None and self.eval_dataset is not None:
                    metrics = self.evaluate()
                    self._report_to_hp_search(trial, epoch + 1, metrics)
                else:
                    # if do not do evaluate, the checkpoint at the end of epoch needs to be saved
                    save_ck(self.args, self.logger, model, tokenizer=self.tokenizer, steps=epoch + 1,
                            tag="epoch")
    ...
    def log(self, logs: Dict[str, float], iterator: Optional[tqdm] = None, tag: str = "") -> None:
        """
        ...
        if self.total_flos is not None:
            ...
            if total_flos > 0:
                logs[f"{tag}_total_flos"] = self.total_flos
        ...
        output = {**logs, **{f"{tag}_global_step": self.global_step}}
        if self.is_world_process_zero():
            self.log_history.append(output)
        if iterator is not None:
            iterator.write(output)
        else:
            self.logger.info(json.dumps(output, indent=2))
    ...
    def _save_tpu(self, output_dir: Optional[str] = None):
        output_dir = output_dir if output_dir is not None else self.args.output_path
    ...
    def _sorted_checkpoints(self, checkpoint_prefix=PREFIX_CHECKPOINT_DIR, use_mtime=False) -> List[str]:
        ...
        glob_checkpoints = [str(x) for x in Path(self.args.output_path).glob(f"{checkpoint_prefix}-*")]
